          ...
          uv.v1.loginAsOwner().then(c => {
            c.get(`forums/${config.uservoice.forumId}/suggestions/${id}.json`).then((data) => {
              c.get(`forums/${config.uservoice.forumId}/suggestions/${id2}.json`).then((data2) => {
                ...
                rl.updateRL(data2.headers['X-Rate-Limit-Limit'], data2.headers['X-Rate-Limit-Remaining'], data2.headers['X-Rate-Limit-Reset'])
              }).catch((e) => {
                if (e.statusCode === 404) {
      ...
      r.db('DFB').table('queue').get(msg.id).then((hj) => {
        ...
        case 'adminMergeRequest': {
          if (reaction.id === '302137375113609219') {
            ...
            merge(doc.UV1, doc.UV2, uv).then(res => {
              rl.updateRL(res.headers['X-Rate-Limit-Limit'], res.headers['X-Rate-Limit-Remaining'], res.headers['X-Rate-Limit-Reset'])
            }).catch((e, body) => {
              logger.log(bot, {
                cause: 'merge_apply',
                message: body
...
function deleteFromUV (UVID, uvClient, bot) {
  uvClient.v1.loginAsOwner().then(i => {
    i.delete(`forums/${config.uservoice.forumId}/suggestions/${UVID}.json`).then(s => {
      rl.updateRL(s.headers['X-Rate-Limit-Limit'],s.headers['X-Rate-Limit-Remaining'],s.headers['X-Rate-Limit-Reset'])
    }).catch((e) => {
      if (e.statusCode === 404) {
